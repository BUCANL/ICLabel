function net = uncompiled_with_workaround(net, inputs)

% Tyler's note:
% Taken from the matconvnet fork by Luca but modified to use
% workaround_bsxfun
disp('Using workaround_bsxfun implementation');
...
for layer_index = net.getLayerExecutionOrder
    ...
    if block_type(block, 'Conv')
        ...
        for it1 = 1:length(hind)
            for it2 = 1:length(vind)
                ...
                output(it1, it2, :, :) = ...
                    sum(reshape(workaround_bsxfun(@times, reshape(patch, ...
        ...
        if block.hasBias
            output = workaround_bsxfun(@plus, output, ...
    ...
    elseif block_type(block, 'SoftMax')
        ...
        output = exp(workaround_bsxfun(@minus, inputs{1}, maxval));
        output = workaround_bsxfun(@rdivide, output, sum(output, 3));
    end
    % save output
